...
already-minted verification token (a link handed to a specific Discord
user for a specific bot-command purpose); `PageGuard` protects an
arbitrary ROUTE -- any page a visitor navigates to, whether or not
they're signed in with Discord yet, including a page that comes
*before* Discord OAuth login even starts (there's no Discord `user_id`
to key anything on at that point, which is exactly the gap this fills).

High-level flow, from a real request during testing ("cloudflare gibi
tam kapasite geniş bir altyapı... sayfaya girişte otomatik ip itibarı...
kullanıcı görmez bile... kötüyse direkt captchaya yollar... ip değişme
sıklığı"):

1. A visitor is identified per request -- the signed-in Discord account
   if there is one, otherwise a random value in an httpOnly cookie
   (minted on first visit).
...
small, optional, composable layer, not wired into
`DiscordWebAPI.install()`. Bring your own `AdaptiveCaptchaGate` (any
`IPReputationChecker`, any `CaptchaProvider`, your own `TrustStore`,
reCAPTCHA/hCaptcha instead of a bundled provider, ...), use `PageGuard`
to sit in front of it, write your own equivalent, or don't gate pages at
all -- nothing here is required to use the rest of `discord_webapi.captcha`.
"""
...
def _pseudo_user_id(raw: str) -> int:
    """A stable, deterministic (NOT a real Discord snowflake) integer
    derived from an arbitrary string -- lets an anonymous visitor's
    cookie value be used anywhere `AdaptiveCaptchaGate`/`TrustStore`
    expect a Discord `user_id: int`, without loosening those Protocols
    to `int | str` everywhere just for this one case. A collision with a
    real Discord id, or between two different anonymous visitors, is
    ...
    `AccountMatchCheck` still requires the *real* signed-in Discord
...
class PageGuard:
    ...
    async def require_human(
        ...
        *,
        ...
        signed-in Discord account, which needs no cookie at all)."""
