...
customizable, hybrid" entry point the rest of `discord_webapi.captcha`
...
from discord_webapi.captcha.checks import VerificationCheck
...
def build_cloudflare_style_guard(
    ...
    *,
    ...
    - `escalation_provider` (default: `PathTraceProvider` -- the
      ...
      -- matching the "default olarak çizgi çizme captchası" request
      this preset was built for). Swap in `ReCaptchaProvider`/
      `HCaptchaProvider`/`TurnstileProvider`/your own for a different
      challenge kind, or a `FallbackCaptchaProvider` composing several.
    - `verification_store`/`decision_store`/`trust_store` (default:
      the in-memory versions of each -- fine for a single process;
      swap in the `discord_webapi.captcha.sql` equivalents once you run
    ...
    - `bind_trust_to_ip` (default `True`, unlike `AdaptiveCaptchaGate`'s
      own default of `False`) -- this preset is specifically the "IP
      değişme sıklığı: bir IP'den bağlandı, başka IP'den bağlanırsa hemen
      captcha" behavior from the request this was built for; pass
      `False` to go back to trust following the account everywhere.
