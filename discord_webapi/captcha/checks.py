...
A captcha alone only proves "a human did this," never "*which* Discord
account did this" -- so a link that's been forwarded to someone else still
solves. That's the whole reason `AccountMatchCheck` exists: it ties the
solve to the actually-signed-in Discord user (via the same OAuth login the
rest of the library uses), which is what makes a verification trustworthy
rather than just "someone, somewhere, filled in a captcha."
...
class AccountMatchCheck:
    """Passes only if the request is authenticated (via the library's own
    Discord OAuth login) *as the exact user the verification was created
    for*. This is what upgrades a verification from "a human did this" to
    "this specific Discord account did this" -- a forwarded link solved by
    someone else fails here."""

    name = "account"

    async def run(self, ctx: VerificationContext) -> CheckOutcome:
        if ctx.authenticated_user_id is None:
            return CheckOutcome(False, "not signed in with Discord")
        if ctx.authenticated_user_id != ctx.request.user_id:
            return CheckOutcome(False, "signed in as a different Discord account")
