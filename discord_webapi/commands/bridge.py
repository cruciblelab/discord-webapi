...
def extract_command_specs(bot: AnyBot) -> list[CommandSpec]:
...
def get_member_roles(bot: AnyBot, guild_id: int, user_id: int) -> list[int] | None:
...
def get_member_permissions(
    bot: AnyBot, guild_id: int, user_id: int
...
def get_channel_permissions(
    bot: AnyBot, guild_id: int, channel_id: int, user_id: int
) -> discord.Permissions | None:
...
def list_manageable_guilds(
    bot: AnyBot, guild_ids: list[int], user_id: int
) -> list[dict[str, Any]]:
