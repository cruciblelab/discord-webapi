...
frontend behavior is exercised manually/with a real browser (see
examples/captcha_playground), not something a unit test can drive."""
...
def test_widget_script_is_served_at_the_default_mount_path() -> None:
    ...
    assert "dwa-captcha-widget" in resp.text
    assert "dwa-captcha-widget-log" in resp.text
